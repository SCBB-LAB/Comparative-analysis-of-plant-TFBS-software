					...
					if learning_steps% 5000==0:
						...
						with torch.no_grad():
							...
							for j, (data1, target1) in enumerate(valid_loader):
								...
								#print("accuracy",(metric(y_pred, y_label)))
...
def Train_model():
	...
	for number_models in range(5):
	  ...
	  with torch.no_grad():
		  ...
		  a=[]
		  prediction1= [x for xs in prediction for x in xs]
		  y_pred=torch.tensor(prediction1)
		  label1= [x for xs in label for x in xs]
		  y_label=torch.tensor(label1)
		  confmat = ConfusionMatrix(task="binary")
		  ...
		  #print("Tr_Auc")
		  AUC_ROC=auroc(y_pred, y_label) 
		  #print("Tr_Acc")
		  accuracy=acc_1(y_pred, y_label)                 
		  #print("tr_CM")
		  confusion=confmat(y_pred, y_label)
		  #print("tr_SP")
		  spf=sp(y_pred, y_label)
		  #print("tr_SN")
		  sen=sn(y_pred, y_label)
		  #print("tr_F1")
		  f1_score=f1(y_pred, y_label)
		  #print("tr__MCC")
		  MCC=mcc(y_pred, y_label)	
		  for i in acc:
		  	tensor_to_list=i.tolist()
		  	a.append(tensor_to_list)
		  model_acc=np.mean(a)
		  #print("tr_acc_mean",model_acc)      
		  AUC_training=np.mean(auc)
		  #print('tr_auc',AUC_training)
		  print('Confusion_matrix on training data for model ',number_models+1,' = ',confusion)
		  print('Accuracy on training data for model ',number_models+1,' = ',accuracy)
		  print('Specificity on training data for model ',number_models+1,' = ',spf)
		  print('Sensitivity on training data for model ',number_models+1,' = ',sen)
		  print('MCC on training data for model ',number_models+1,' = ',MCC)
		  print('F1-score on training data for model ',number_models+1,' = ',f1_score)
...
def test_predict():
	...
	with torch.no_grad():
		  ...
		  if evaluate_performance:                     
			  ...
			  accuracy=acc_1(y_pred, y_label)                 
			  #print("tr_CM")
			  conf_mat=confmat(y_pred, y_label)
			  #print("tr_SP")
			  specificity=sp(y_pred, y_label)
			  #print("tr_SN")
			  sensitivity=sn(y_pred, y_label)
			  #print("tr_F1")
			  f1_score=f1(y_pred, y_label)
			  #print("tr__MCC")
			  MCC=mcc(y_pred, y_label)	
			  for i in acc:
			  	...
			  	a.append(tensor_to_list)
			  model_acc=np.mean(a)
			  TN=(conf_mat[0][0].tolist())
			  FP=(conf_mat[0][1].tolist())
			  FN=(conf_mat[1][0].tolist())
			  TP=(conf_mat[1][1].tolist())
			  
			  fw.writelines("Test"+"\t"+"TP"+"\t"+"TN"+"\t"+"FN"+"\t"+"FP"+"\t"+"TPR"+"\t"+"TNR"+"\t"+"ACC"+"\t"+"F1"+"\t"+"MCC"+"\t"+"auc"+"\n")
			  fw.writelines("\t"+str(TP)+"\t"+str(TN)+"\t"+str(FN)+"\t"+str(FP)+"\t"+str(sensitivity.item())+"\t"+str(specificity.item())+"\t"+str(accuracy.item())+"\t"+str(f1_score.item())+"\t"+str(MCC.item())+"\t"+str(AUC_ROC)+"\n")
			  fw.close()
...
word2vec_model='output/word2vec_model'
...
## model
    ...
    parser.add_argument('--out_file1', type=str, default='result.txt',help='The output file used to store the performance matrix of testing data')
