...
def plot_filter_logo(filter_outs, filter_size, seqs, out_prefix, raw_t=0, maxpct_t=None):
    ...
    # make weblogo
    if filter_count > 0:
        weblogo_cmd = 'weblogo --format PDF %s < %s.fa > %s.pdf' % (weblogo_opts, out_prefix, out_prefix)
...
def get_motif_fig(filter_weights, filter_outs, out_dir, seqs, sample_i = 0):
    ...
    for f in range(num_filters):
        ...
        filter_pwm_file = '%s/pwm%d.txt' % (out_dir, f)
        #with open(filter_pwm_file, 'w') as meme_out:
        #    filter_pwm, nsites = make_filter_pwm('%s/filter%d_logo.fa' % (out_dir, f))
            #meme_add(meme_out, filter_pwm, nsites, False)
        
        meme_out = meme_intro(filter_pwm_file, seqs)
        ...
        filter_pwm, nsites = make_filter_pwm('%s/filter%d_logo.fa'%(out_dir,f))
       # meme_add(meme_out, 0, filter_pwm, nsites, False)
#        print(filter_pwm)

        
        with open('indices.txt', 'a') as indices_file:
    ...
    if data_type=='RNA':
        ...
        subprocess.call(tomtom_dir+' -dist pearson -thresh 0.05 -oc %s/tomtom %s/filters_meme.txt %s' % (out_dir, out_dir, '/home/scbb/Desktop/tf_motifs/JASPAR2022_CORE_plants_non-redundant_v2.meme'), shell=True)
    ...
    if data_type=='RNA':
        ...
        filter_names = name_filters(num_filters, '%s/tomtom/tomtom.txt'%out_dir, '/home/scbb/Desktop/tf_motifs/JASPAR2022_CORE_plants_non-redundant_v2.meme')
