...
def plot_filter_logo(filter_outs, filter_size, seqs, out_prefix, raw_t=0, maxpct_t=None):
    ...
    # make weblogo
    if filter_count > 0:
        #weblogo_cmd = 'weblogo %s --format PDF < %s.fa > %s.pdf' % (weblogo_opts, out_prefix, out_prefix)
        weblogo_cmd = 'weblogo %s --format PDF -A dna -C "#CB2026" A A -C "#34459C" C C -C "#FBB116" G G -C "#0C8040" T T --errorbars NO < %s.fa > %s.pdf' % (weblogo_opts, out_prefix, out_prefix)
